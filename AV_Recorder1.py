...
def av_record():
    ...
    if os.path.isfile('test1.wav'): os.remove('test1.wav') 
    if os.path.isfile('test1.mp4'): os.remove('test1.mp4') 
    if os.path.isfile('output1.mp4'): os.remove('output1.mp4')

    video_thread.save_output()
    audio_thread.save_output()
    
    # merging audio and video
    cmd = 'ffmpeg -i test1.mp4 -i test1.wav -c:v copy -c:a aac output1.mp4'
